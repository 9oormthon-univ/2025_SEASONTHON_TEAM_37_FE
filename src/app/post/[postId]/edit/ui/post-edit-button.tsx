...
const PostEditButton = ({ className }: { className?: string }) => {
  ...
  const editPostMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast.success('실패담이 수정되었습니다.');
      router.replace(PATH.post.detail(+postId));
    },
    onError: () => {
      toast.error('실패담 수정에 실패했습니다.');
    },
  });

  const onEdit = (data: PostEditFormData) => {
    const payload = {
      ...data,
  ...
  return (
    <Button
      ...
      onClick={handleSubmit(onEdit, onEditPostInvalid)}
      variant="outline"
